...
# 1. PAGE SETUP & THEME (Field Green and Cream)
...
# 4. LOGIN INTERFACE (Sidebar)
st.sidebar.title("🏢 Mainland Group")
user_input = st.sidebar.text_input("Login (First Name or Admin Password)", type="password")

if st.sidebar.button("Login"):
    if user_input == "MainlandTep":
        st.session_state.logged_in = True
        st.session_state.role = "Admin"
    elif any(user_input.lower() in name.lower() for name in staff_list):
        full_name = next(name for name in staff_list if user_input.lower() in name.lower())
        st.session_state.logged_in = True
        st.session_state.role = "User"
        st.session_state.user_name = full_name
    else:
        st.sidebar.error("Access Denied. Check spelling.")
...
if st.session_state.logged_in and st.session_state.role == "User":
    ...
    if choice == "Submit Work Plan":
        ...
        with st.form("user_form"):
            s_name = st.selectbox("Staff Name", staff_list, index=staff_list.index(st.session_state.user_name))
            ...
            if submit_btn:
                if confirm:
                    new_entry = pd.DataFrame([{
                        "Staff Name": s_name, "Date": str(s_date), "Work Plan": s_plan,
...
if st.session_state.logged_in and st.session_state.role == "Admin":
    ...
    if pending.empty:
        st.write("No pending submissions to review.")
    else:
